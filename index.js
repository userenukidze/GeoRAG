...
const pc = new Pinecone({
  apiKey: 'pcsk_TqZYd_2JKFQdA9hNpduqVHPx2E6Xo5LfQsLNZFRjXMDu4jnWWrtdpNkitXNs96cTHWUec'
});
...
async function embedChunks(chunks) {
    ...
    for (let i = 0; i < chunks.length; i++) {
        ...
        const output = await embedder(chunks[i].text, { pooling: 'cls', normalize: true });
...
async function searchQuery(query, topK = 5) {
    ...
    const embedder = await pipeline('feature-extraction', 'Xenova/bge-m3');
    const queryEmbedding = (await embedder(query, { pooling: 'cls', normalize: true })).data;
